...
def setup():
    ...
    env.add_connections({0:range((NUM_AGENTS*NUM_COPIES+3))}, load_param_dict=loadParams)
...
response = env.reset()
means = np.array([response[1][i][0][2] for i in range(NUM_COPIES)])
vars = np.array([response[1][i][0][3] for i in range(NUM_COPIES)])


rewards = np.array(rewards)
colors = ['y','b','g','r','k','r:']
capacity_rewards = np.array([(capacity_function(i),j) for i,j in enumerate(rewards[-1][:-3])])
charging_rewards = np.array([(charging_rate_function(i),j) for i,j in enumerate(rewards[-1][:-3])])
capacity_rewards_sorted = np.array(sorted([(capacity_function(i),j) for i,j in enumerate(rewards[-1][:-3])], key = lambda x: x[1]))
plt.plot(capacity_rewards[:,0], capacity_rewards[:,1], label = 'Total Cost Savings after %d days' % NUM_DAYS)
...
plt.legend()
# plt.axes().set(title="Agent Performance",xlabel = "Battery Capacity (in kWh)")
plt.axes().set(title="Agent Performance",xlabel = "Battery Capacity (in kWh)")

# for i in range(NUM_AGENTS):
#     # plt.figure(i)
#     for j in range(0,5):
#         plt.plot(range(len(rewards)),rewards[:,i*NUM_COPIES+j], label=str(capacity_function(j)))
# plt.legend()
# for i in range(3):
#     plt.plot(range(len(rewards)), rewards[:, NUM_AGENTS* NUM_COPIES + i], colors[i+NUM_AGENTS])
#
